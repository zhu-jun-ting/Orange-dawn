...
public class EventNPCDialogue : MonoBehaviour
{
    ...
    private void HandleFKeyPressed()
    {
        ...
        {
            // Set a random float parameter between 0-1 before starting conversation using ConversationManager syntax
            float randomValue = UnityEngine.Random.value;
            // If you have SetFloat, use: ConversationManager.Instance.SetFloat("RandomValue", randomValue);
            ConversationManager.Instance.StartConversation(conversation);
            ConversationManager.Instance.SetInt("RandomValue", Mathf.RoundToInt(randomValue * 100)); // Store as int (0-100)
            ConversationManager.Instance.SetInt("coinCurrent", CoinCounter.coinCurrent);
        }
    }
