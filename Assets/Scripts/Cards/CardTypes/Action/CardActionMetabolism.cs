...
// id: custom
// name: Metabolism
// desc: When you take damage, Probability: 30% to create a bullet Accelerator or Cloner. Cost Coin: 5
public class CardActionMetabolism : CardMaster, ICardAction
{
    ...
    private void HandleOnHitPawn(float damage, PawnMaster receiver, GameObject instigator, GameEvents.DamageType damageType, Transform location, float hitBack, Gun source)
    {
        ...
        if (damage <= 0) return; // Only trigger on damage
    ...
    {
        return GameSettings.AddIcon(string.Format(card_description, probability, (int)coin));
    }
}
